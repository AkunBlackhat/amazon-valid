...
func main() {
	sad := `   ___                                         _    __      ___     __
   /   |  ____ ___  ____ _____  ____  ____     | |  / /___ _/ (_)___/ /`

	fmt.Println(sad)
	fmt.Println("@AkunBlackhat")
	fmt.Println("")
	...
	go func() {
		...
		close(jobs)
		color.New(color.FgGreen, color.BgBlack).Println("Done.")
		os.Exit(2)
	}()
	...
	for w := 1; w <= 20; w++ {
		wg.Add(20)
		// fmt.Println(lines)
		go run(jobs, wg, results, totalData)
	}
...
func run(email <-chan string, wg *sync.WaitGroup, results <-chan int, totalData int) {
	defer wg.Done()
	// line := 1
	for j := range email {
		// index := getIndex(totalData)
		...
		if strings.Contains(result, "but an account already exists with the e-mail") {
			...
			color.New(color.FgRed, color.BgBlack).Println("DIE : " + j)
			createFile(j, "")
		}
		// line++
	}
...
func getIndex(totalData int) int {
	var line int
...
func createFile(email string, indicator string) {
	...
	if strings.Contains(indicator, "but an account already exists with the e-mail") {
		...
		f, err := os.OpenFile("DIE.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
		if err != nil {
			log.Fatal(err)
		}
		if _, err := f.Write([]byte(newEmail)); err != nil {
